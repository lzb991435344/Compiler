...
//AMD PCnet
//TechDocs, page 186-188
//http://support.amd.com/TechDocs/20550.pdf
...
amd_am79c973::amd_am79c973(PeripherComponentInterconnectDeviceDescriptor* dev,  
...
InterruptHandler(dev->interrupt + interruptManager->HardwareInterruptOffset, interruptManager),
	...
	**/
		...
		recvBufferDescr = (BufferDescriptor*)((((uint32_t)&sendBufferDescrMemory[0]) + 15) & ~((uint32_t)0xF));
		initBlock.recvBufferDescrAddress = (uint32_t)recvBufferDescr;

		for(uint8_t i = 0; i < 8; ++i){
			sendBufferDescr[i].address = (((uint32_t)(&sendBufferDescrMemory[0]) + 15) & ~(uint32_t)0xF);
			sendBufferDescr[i].flags = 0x7FF | 0xF000;
			sendBufferDescr[i].flags2 = 0;
			sendBufferDescr[i].avail = 0;


			recvBufferDescr[i].address = (((uint32_t)(&recvBufferDescrMemory[0]) + 15) & ~(uint32_t)0xF);
...
uint32_t amd_am79c973::handlerInterrupt(uint32_t esp){
	printf("INTERRUPT FROM AMD amd_am79c973\n");

	registerAddressPort.Write(0);
	uint32_t temp = registerDataPort.Read();

	if((temp & 0x8000) == 0x8000)  printf("AMD amd_am79c973 ERROR\n");
	if((temp & 0x2000) == 0x2000)  printf("AMD amd_am79c973  COLLISION ERROR\n");
	if((temp & 0x1000) == 0x1000)  printf("AMD amd_am79c973 MISSED FRAME\n");
	if((temp & 0x0800) == 0x0800)  printf("AMD amd_am79c973  MEMORY ERROR\n");
	if((temp & 0x0400) == 0x0400)  Receive();
	if((temp & 0x0200) == 0x0200)  printf("AMD amd_am79c973  DATA SEND\n");


	registerAddressPort.Write(0);
	registerDataPort.Write(temp);

	if((temp & 0x0200) == 0x0200)  printf("AMD amd_am79c973  INIT DONE\n"); 
...
//20550.pdf 184 
//first line check the Error Bit 
//second lien check STP and ENP (start and the end of package)
void amd_am79c973::Receive(){

	printf("AMD amd_am79c973  DATA Receive\n");
